...
highlight = c('PAB1')
highlight_orfs = unique(df |> filter(gene %in% highlight) |> pull(ORF))
p_prot_vs_mRNA_condensation = ggplot(drp, aes(x=Treatment, y=pSup, group=Treatment, color=Temperature)) +
  ...
  geom_line(data=drp |> filter(ORF %in% highlight_orfs, Treatment %in% paste('prot',Temperature,sep='.')), 
            aes(x=factor(Treatment), group=1), colour=blackcol, linewidth=1) +
  geom_point(data=drp |> filter(ORF %in% highlight_orfs, Treatment %in% paste('prot',Temperature,sep='.')), fill="white", colour=blackcol, size=3, shape=21) +
...
```{r khong_comparison, dependson="load_data", fig.asp=0.5, fig.width=8}
# Khong et al. data (Parker lab)
kho = read_tsv(file.path(github_dir,"src/khong-parker-2017-yeast-sg-transcriptome-tableS2.txt"), comment='#')

dat_azide = df_Zsup_mean |> filter(Treatment_group=='azide3') |> mutate(length=LengthTxEst)
pskho = dat_azide |> filter(Treatment=='Azide_0.8%_pH6.8') |> select(ORF, gene, length, pSup.mean, sed.mean, esc.mean) |> left_join(kho |> select(-gene, -length), by='ORF')
pskho_mock = dat_azide |> filter(Treatment=='mock') |> select(ORF, gene, length, pSup.mean, sed.mean, esc.mean) |> left_join(kho |> select(-gene, -length), by='ORF') 

p_khong_vs_sed = ggplot(pskho, aes(x=foldchange.sg.total, y=sed.mean, colour=sg.classification)) + 
  ggrastr::rasterize(geom_point(shape=shap, alpha=alph),dpi=600) +
  scale_y_continuous("Differential sedimentation (Δsed)", limits=c(-2,6.5)) + 
  scale_x_log2nice("Stress granule enrichment (Khong *et al.*)", limits=c(0.125,8)) +
  scale_colour_manual(values=sg_enrichment_colors) +
  guides(colour = guide_legend(override.aes = list(alpha = 1))) + no.legend + larger_labels +
  theme(axis.title.x = ggtext::element_markdown())
p_khong_vs_pSup_mock = ggplot(pskho_mock, aes(x=foldchange.sg.total, y=1-pSup.mean, colour=sg.classification)) + 
  ggrastr::rasterize(geom_point(shape=shap, alpha=alph),dpi=600) + 
  scale_y_log2nice(expression(paste("Proportion in pellet ", (1-pSup),"\nmock treatment")), limits=2^c(-4.5,0)) + 
  scale_x_log2nice("Stress granule enrichment (Khong *et al.*)", limits=c(0.125,8)) +
  scale_colour_manual(values=sg_enrichment_colors) +
  guides(colour = guide_legend(override.aes = list(alpha = 1))) + no.legend + larger_labels +
  theme(axis.title.x = ggtext::element_markdown())

cor.test(pskho$sed.mean, pskho$foldchange.sg.total, method='s')
cor.test(1-pskho_mock$pSup.mean, pskho_mock$foldchange.sg.total, method='s')
p_khong = p_khong_vs_sed + p_khong_vs_pSup_mock
print(p_khong)
...
ggsave(file.path(github_dir, "figures/FigureS1/FigS1_khong_comparison.pdf"),
       p_khong,
       width = fig.width*fig.vs.panel.ratio,
       height = fig.width*fig.asp*fig.vs.panel.ratio,
       units = "mm", bg = "transparent", create.dir=TRUE, device=cairo_pdf)
```
...
p_violin <- ggplot(df,
  ...
  scale_x_discrete(labels=plot_group_labels)+
  #scale_y_continuous(limits = c(sedMax,sedMin), breaks = c(10,5,0), transform = "reverse") +
  scale_y_continuous(limits = c(sedMin,sedMax), breaks = c(0, 5, 10))+
  scale_fill_manual(values=stress.cols)+
  scale_alpha_manual(values=c("mock"=0.8,
                   "42C"=0.8,
                   "46C"=0.8,
...
# Figure 3F: stress-specific escape of transcripts in stresses it is induced in: heat shock vs etoh.

Using escape scores (eSed).

```{r}
num_most_induced_genes <- 100 #number of most induced genes to use in analysis

#per JB, esc score is already calculated for the stress condition (ie ignore that in control - effectively noise)
#azide 0.8%
df_azide_08_delta <- df_Zsup_mean %>% filter(Treatment_group %in% c('azide3')) %>%
  select(ORF,gene, LengthTxEst,Strain,Treatment_group,Treatment, Temperature, Total.TPM.mean,Zsup.mean,esc.mean) %>%
  pivot_wider(names_from = c(Treatment,Temperature), values_from = c(Total.TPM.mean,Zsup.mean,esc.mean)) %>%
  mutate(tpm_fc_azide_08 = as.numeric(`Total.TPM.mean_Azide_0.8%_pH6.8_30C`) / as.numeric(Total.TPM.mean_mock_30C),
         Zsup_delta_azide_08 = as.numeric(`Zsup.mean_Azide_0.8%_pH6.8_30C`) - as.numeric(Zsup.mean_mock_30C),
         esed_azide_08 = as.numeric(`esc.mean_Azide_0.8%_pH6.8_30C`))
# #test 0.5% azide data frame:
# df_azide_08_delta <- df_Zsup_mean %>% filter(Treatment_group %in% c('Azide_0.5%_pH6.8')) %>%
#   select(ORF,gene, LengthTxEst,Strain,Treatment_group,Treatment, Temperature, Total.TPM.mean,Zsup.mean,esc.mean) %>%
#   pivot_wider(names_from = c(Treatment,Temperature), values_from = c(Total.TPM.mean,Zsup.mean,esc.mean)) %>%
#   mutate(tpm_fc_azide_08 = as.numeric(`Total.TPM.mean_Azide_0.5%_pH6.8_30C`) / as.numeric(Total.TPM.mean_mock_30C),
#          Zsup_delta_azide_08 = as.numeric(`Zsup.mean_Azide_0.5%_pH6.8_30C`) - as.numeric(Zsup.mean_mock_30C),
#          esed_azide_08 = as.numeric(`esc.mean_Azide_0.5%_pH6.8_30C`))
#etoh 7.5%
df_etoh_75_delta <- df_Zsup_mean %>% filter(Treatment_group %in% c('milderEtOH2')) %>%
  select(ORF,gene, LengthTxEst,Strain,Treatment_group,Treatment, Temperature, Total.TPM.mean,Zsup.mean,esc.mean) %>%
  pivot_wider(names_from = c(Treatment,Temperature), values_from = c(Total.TPM.mean,Zsup.mean,esc.mean)) %>%
  mutate(tpm_fc_etoh_75 = as.numeric(`Total.TPM.mean_EtOH_7.5%_30C`) / as.numeric(Total.TPM.mean_mock_30C),
         Zsup_delta_etoh_75 = as.numeric(`Zsup.mean_EtOH_7.5%_30C`) - as.numeric(Zsup.mean_mock_30C),
         esed_etoh_75 = as.numeric(`esc.mean_EtOH_7.5%_30C`))
#42C
df_temp_42_delta <- df_Zsup_mean %>% filter(Treatment_group %in% c('EW_TSPP') & Temperature %in% c('30C','42C')) %>%
  select(ORF,gene, LengthTxEst,Strain,Treatment_group,Treatment, Temperature, Total.TPM.mean,Zsup.mean,esc.mean) %>%
  pivot_wider(names_from = c(Treatment,Temperature), values_from = c(Total.TPM.mean,Zsup.mean,esc.mean)) %>%
  mutate(tpm_fc_temp_42 = as.numeric(`Total.TPM.mean_none_42C`) / as.numeric(`Total.TPM.mean_none_30C`),
         Zsup_delta_temp_42 = as.numeric(`Zsup.mean_none_42C`) - as.numeric(Zsup.mean_none_30C),
         esed_temp_42 = as.numeric(`esc.mean_none_42C`))

#standardize naming for rbind
df_temp_42_delta <- df_temp_42_delta %>%
  select(ORF,LengthTxEst,tpm_fc_temp_42,Zsup_delta_temp_42,esed_temp_42) %>%
  mutate(Treatment = 'Temp_42C') %>%
  dplyr::rename(tpm_fc = tpm_fc_temp_42, Zsup_delta = Zsup_delta_temp_42, esed = esed_temp_42)

df_azide_08_delta <- df_azide_08_delta %>%
  select(ORF,LengthTxEst,tpm_fc_azide_08,Zsup_delta_azide_08,esed_azide_08) %>%
  mutate(Treatment = 'Azide_08') %>%
  dplyr::rename(tpm_fc = tpm_fc_azide_08, Zsup_delta = Zsup_delta_azide_08, esed = esed_azide_08)

df_etoh_75_delta <- df_etoh_75_delta %>%
  select(ORF,LengthTxEst,tpm_fc_etoh_75,Zsup_delta_etoh_75, esed_etoh_75) %>%
  mutate(Treatment = 'Ethanol_75') %>%
  dplyr::rename(tpm_fc = tpm_fc_etoh_75, Zsup_delta = Zsup_delta_etoh_75, esed = esed_etoh_75)

#now rbind all of these
df_combineStress_cum <- rbind(df_azide_08_delta, df_etoh_75_delta) %>%
  rbind(.,df_temp_42_delta)

#compare delta Zsup's
df_combineStress_wide <- df_combineStress_cum %>% pivot_wider(data = ., names_from = c(Treatment), values_from = c(tpm_fc,Zsup_delta,esed))

#get top X transcripts induced each stress (X set by variable num_most_induced_genes at top)
df_topInduced <- df_combineStress_cum %>% arrange(desc(tpm_fc)) %>%
  group_by(Treatment) %>%
  slice(1:num_most_induced_genes)
etoh_topInduced <- df_topInduced %>% filter(Treatment %in% c('Ethanol_75')) %>% ungroup() %>% select(ORF)
temp_topInduced <- df_topInduced %>% filter(Treatment %in% c('Temp_42C')) %>% ungroup() %>% select(ORF)
azide_topInduced <- df_topInduced %>% filter(Treatment %in% c('Azide_08')) %>% ungroup() %>% select(ORF)
etoh_topInduced <- etoh_topInduced$ORF
temp_topInduced <- temp_topInduced$ORF
azide_topInduced <- azide_topInduced$ORF
df_combineStress_wide <- df_combineStress_wide %>%
  mutate(temp_induced = NA,
         etoh_induced = NA,
         azide_induced = NA,
         inducedCategory = NA)

df_combineStress_wide$temp_induced[which(df_combineStress_wide$ORF %in% temp_topInduced)] <- 'induced'
df_combineStress_wide$temp_induced[which(!(df_combineStress_wide$ORF %in% temp_topInduced))] <- 'other'
df_combineStress_wide$etoh_induced[which(df_combineStress_wide$ORF %in% etoh_topInduced)] <- 'induced'
df_combineStress_wide$etoh_induced[which(!(df_combineStress_wide$ORF %in% etoh_topInduced))] <- 'other'
df_combineStress_wide$azide_induced[which(df_combineStress_wide$ORF %in% azide_topInduced)] <- 'induced'
df_combineStress_wide$azide_induced[which(!(df_combineStress_wide$ORF %in% azide_topInduced))] <- 'other'

#now just focus on heat and ethanol
df_heatEtOHStress_wide <- df_combineStress_wide
#redo labels for just heat and etoh (rather than all 3) -> need to use wide to do
#now do overall induced category
df_heatEtOHStress_wide$inducedCategory[which(df_heatEtOHStress_wide$temp_induced %in% c('induced') &
                                              df_heatEtOHStress_wide$etoh_induced %in% c('induced'))] <- 'temp,etoh'
df_heatEtOHStress_wide$inducedCategory[which(df_heatEtOHStress_wide$temp_induced %in% c('induced') &
                                              df_heatEtOHStress_wide$etoh_induced %in% c('other'))] <- 'temp'
df_heatEtOHStress_wide$inducedCategory[which(df_heatEtOHStress_wide$temp_induced %in% c('other') &
                                              df_heatEtOHStress_wide$etoh_induced %in% c('induced'))] <- 'etoh'
df_heatEtOHStress_wide$inducedCategory[which(df_heatEtOHStress_wide$temp_induced %in% c('other') &
                                              df_heatEtOHStress_wide$etoh_induced %in% c('other'))] <- 'other'

#make long:
df_heatEtOHStress_long <- df_heatEtOHStress_wide %>%
  select(ORF,esed_Azide_08,esed_Ethanol_75,esed_Temp_42C,inducedCategory) %>%
  pivot_longer(., cols = c("esed_Azide_08","esed_Ethanol_75","esed_Temp_42C"), 
               names_to = "Stress", values_to = "esed")

df_heatEtOHStress_long$Stress[which(df_heatEtOHStress_long$Stress == "esed_Azide_08")] <- "0.8% Azide"
df_heatEtOHStress_long$Stress[which(df_heatEtOHStress_long$Stress == "esed_Temp_42C")] <- "42°C"
df_heatEtOHStress_long$Stress[which(df_heatEtOHStress_long$Stress == "esed_Ethanol_75")] <- "7.5% Ethanol"
df_heatEtOHStress_long$Stress <- factor(df_heatEtOHStress_long$Stress, levels = c("42°C","7.5% Ethanol","0.8% Azide"))

#factoring
df_heatEtOHStress_long$inducedCategory <- factor(df_heatEtOHStress_long$inducedCategory,
                                                    levels = c("other", "temp", "etoh", "azide",
                                                               "temp,etoh", "temp,azide", "etoh,azide", "temp,etoh,azide"))

#column plot with sd
#bar graph version where bar graph doesn't start at 0, involves arbitrarily resetting the 0 and then relabeling axes
# p_stressSpecificEscape_heatEtOH <- ggplot(data = df_heatEtOHStress_long %>% filter(Stress %in% c('42°C','7.5% Ethanol')),
#        aes(y = Zsup_delta + 4, x = inducedCategory, fill = inducedCategory)) +
#   geom_jitter(shape=16,height = 0, alpha = 0.3) +
...
#                geom = "linerange",
#                alpha = .8) +
#   scale_x_discrete(name = 'Induction Category',
#                    labels = c('other' = 'Neither',
#                               'temp' = 'Temp',
#                               'etoh' = 'Ethanol',
#                               'azide' = 'Azide',
#                               'temp,etoh' = 'Both',
#                               'temp,azide' = 'Temp Azide',
#                               'etoh,azide' = 'Ethanol Azide',
#                               'temp,etoh,azide' = 'Temp Ethanol Azide')) +
#   scale_y_continuous(name = '∆Zsup (stress)', limits = c(0,8),
#                      labels=seq(0,8,2) - 4, expand = c(0,0)) +
#   scale_fill_manual(values = c('grey','orange','#A155B9','purple')) +
#   facet_grid(cols = vars(Stress)) +
#   theme(legend.position = 'none')
# p_stressSpecificEscape_heatEtOH

#calculate the summary stats:
df_heatEtOHStress_summary <- df_heatEtOHStress_long %>%
  select(-ORF) %>%
  filter(Stress %in% c('42°C','7.5% Ethanol') & !is.na(esed)) %>%
  group_by(Stress, inducedCategory) %>%
  summarise(n = n(),
            mean_esed = ave(esed),
            sd_esed = sd(esed),
            se_esed = sd(esed)/sqrt(n())) %>%
  unique()

#plotting means +/-standard error
p_stressSpecificEscape_heatEtOH <- ggplot(
  data = df_heatEtOHStress_long %>% 
    filter(Stress %in% c('42°C','7.5% Ethanol')),
  aes(y = esed, x = inducedCategory, fill = inducedCategory)
) +
  ggrastr::rasterize(geom_jitter(data = df_heatEtOHStress_long %>% filter(Stress %in% c('42°C','7.5% Ethanol'), inducedCategory == "other"),
    shape = 16,height = 0,alpha = 0.05, size = 0.5),dpi=600) +
 ggrastr::rasterize(geom_jitter(data = df_heatEtOHStress_long %>% filter(Stress %in% c('42°C','7.5% Ethanol'), inducedCategory != "other"),
    shape = 16,height = 0,alpha = 0.3, size = 0.5, aes(color = inducedCategory), width = 0.25),dpi=600) +
  geom_hline(yintercept = 0, linetype = 'dashed') +
  geom_col(data = df_heatEtOHStress_summary,
           aes(y = mean_esed),
           color = 'black',
           alpha = 0.8) +
  geom_errorbar(data = df_heatEtOHStress_summary,
           aes(y = mean_esed, ymin = mean_esed - se_esed, ymax = mean_esed + se_esed),
           color = 'black',
           alpha = 1,
           width = 0.2) +
  scale_x_discrete(name = 'Induction Category',
                   labels = c('other' = 'Neither',
                              ...
                              'etoh,azide' = 'Ethanol Azide',
                              'temp,etoh,azide' = 'Temp Ethanol Azide')) +
  scale_y_continuous(name = 'Escape score (stress)',
                     limits = c(-2,3)) +
  scale_fill_manual(values = c('grey',RColorBrewer::brewer.pal(n=7,"Oranges")[4],
                               RColorBrewer::brewer.pal(n=7,"Purples")[5],
                               '#AD3D6F')) +
  scale_color_manual(values = c(RColorBrewer::brewer.pal(n=7,"Oranges")[4],RColorBrewer::brewer.pal(n=7,"Purples")[5],'#AD3D6F')) +
  facet_grid(cols = vars(Stress)) +
  theme(legend.position = 'none')

p_stressSpecificEscape_heatEtOH

ggsave(file.path(github_dir,"figures","Figure3/stressSpecificEscape_heatEtoh.pdf"),
       p_stressSpecificEscape_heatEtOH,
       width=100,height=50,units="mm",bg="transparent", create.dir=TRUE)

```

# Try azide again, this time with escape scores and top 100 transcripts.


```{r}
#now just focus on heat and ethanol
df_heatAzideStress_wide <- df_combineStress_wide
#redo labels for just heat and etoh (rather than all 3) -> need to use wide to do
#now do overall induced category
df_heatAzideStress_wide$inducedCategory[which(df_heatAzideStress_wide$temp_induced %in% c('induced') &
                                              df_heatAzideStress_wide$azide_induced %in% c('induced'))] <- 'temp,azide'
df_heatAzideStress_wide$inducedCategory[which(df_heatAzideStress_wide$temp_induced %in% c('induced') &
                                              df_heatAzideStress_wide$azide_induced %in% c('other'))] <- 'temp'
df_heatAzideStress_wide$inducedCategory[which(df_heatAzideStress_wide$temp_induced %in% c('other') &
                                              df_heatAzideStress_wide$azide_induced %in% c('induced'))] <- 'azide'
df_heatAzideStress_wide$inducedCategory[which(df_heatAzideStress_wide$temp_induced %in% c('other') &
                                              df_heatAzideStress_wide$azide_induced %in% c('other'))] <- 'other'

#make long:
df_heatAzideStress_long <- df_heatAzideStress_wide %>%
  select(ORF,esed_Azide_08,esed_Ethanol_75,esed_Temp_42C,inducedCategory) %>%
  pivot_longer(., cols = c("esed_Azide_08","esed_Ethanol_75","esed_Temp_42C"), 
               names_to = "Stress", values_to = "esed")

df_heatAzideStress_long$Stress[which(df_heatAzideStress_long$Stress == "esed_Azide_08")] <- "0.8% Azide"
df_heatAzideStress_long$Stress[which(df_heatAzideStress_long$Stress == "esed_Temp_42C")] <- "42°C"
df_heatAzideStress_long$Stress[which(df_heatAzideStress_long$Stress == "esed_Ethanol_75")] <- "7.5% Ethanol"
df_heatAzideStress_long$Stress <- factor(df_heatAzideStress_long$Stress, levels = c("42°C","7.5% Ethanol","0.8% Azide"))

#factoring
df_heatAzideStress_long$inducedCategory <- factor(df_heatAzideStress_long$inducedCategory,
                                                    levels = c("other", "temp", "etoh", "azide",
                                                               "temp,etoh", "temp,azide", "etoh,azide", "temp,etoh,azide"))

#calculate the summary stats:
df_heatAzideStress_summary <- df_heatAzideStress_long %>%
  select(-ORF) %>%
  filter(Stress %in% c('42°C','0.8% Azide') & !is.na(esed)) %>%
  group_by(Stress, inducedCategory) %>%
  summarise(n = n(),
            mean_esed = ave(esed),
            sd_esed = sd(esed),
...
#plotting means +/-standard error
p_stressSpecificEscape_heatAzide <- ggplot(data = df_heatAzideStress_long %>% filter(Stress %in% c('42°C','0.8% Azide')),
       aes(y = esed, x = inducedCategory, fill = inducedCategory)) +
  geom_jitter(shape=16, height = 0, alpha = 0.03) +
  geom_hline(yintercept = 0, linetype = 'dashed') +
  geom_col(data = df_heatAzideStress_summary,
           aes(y = mean_esed),
           color = 'black',
           alpha = 0.8) +
  geom_errorbar(data = df_heatAzideStress_summary,
           aes(y = mean_esed, ymin = mean_esed - se_esed, ymax = mean_esed + se_esed),
           color = 'black',
           alpha = 1,
           width = 0.2) +
  ...
  scale_y_continuous(name = 'Escape score (stress)',
                     limits = c(-2,3)) +
  scale_fill_manual(values = c('grey','orange','forestgreen','brown')) +
  facet_grid(cols = vars(Stress)) +
  theme(legend.position = 'none')
p_stressSpecificEscape_heatAzide
...
```{r}
#now just focus on heat and ethanol
df_etohAzideStress_wide <- df_combineStress_wide
#redo labels for just heat and etoh (rather than all 3) -> need to use wide to do
#now do overall induced category
df_etohAzideStress_wide$inducedCategory[which(df_etohAzideStress_wide$etoh_induced %in% c('induced') &
                                              df_etohAzideStress_wide$azide_induced %in% c('induced'))] <- 'etoh,azide'
df_etohAzideStress_wide$inducedCategory[which(df_etohAzideStress_wide$etoh_induced %in% c('induced') &
                                              df_etohAzideStress_wide$azide_induced %in% c('other'))] <- 'etoh'
df_etohAzideStress_wide$inducedCategory[which(df_etohAzideStress_wide$etoh_induced %in% c('other') &
                                              df_etohAzideStress_wide$azide_induced %in% c('induced'))] <- 'azide'
df_etohAzideStress_wide$inducedCategory[which(df_etohAzideStress_wide$etoh_induced %in% c('other') &
                                              df_etohAzideStress_wide$azide_induced %in% c('other'))] <- 'other'

#make long:
df_etohAzideStress_long <- df_etohAzideStress_wide %>%
  select(ORF,esed_Azide_08,esed_Ethanol_75,esed_Temp_42C,inducedCategory) %>%
  pivot_longer(., cols = c("esed_Azide_08","esed_Ethanol_75","esed_Temp_42C"), 
               names_to = "Stress", values_to = "esed")

df_etohAzideStress_long$Stress[which(df_etohAzideStress_long$Stress == "esed_Azide_08")] <- "0.8% Azide"
df_etohAzideStress_long$Stress[which(df_etohAzideStress_long$Stress == "esed_Temp_42C")] <- "42°C"
df_etohAzideStress_long$Stress[which(df_etohAzideStress_long$Stress == "esed_Ethanol_75")] <- "7.5% Ethanol"
df_etohAzideStress_long$Stress <- factor(df_etohAzideStress_long$Stress, levels = c("42°C","7.5% Ethanol","0.8% Azide"))

#factoring
df_etohAzideStress_long$inducedCategory <- factor(df_etohAzideStress_long$inducedCategory,
                                                    levels = c("other", "temp", "etoh", "azide",
                                                               "temp,etoh", "temp,azide", "etoh,azide", "temp,etoh,azide"))

#calculate the summary stats:
df_etohAzideStress_summary <- df_etohAzideStress_long %>%
  select(-ORF) %>%
  filter(Stress %in% c('7.5% Ethanol','0.8% Azide') & !is.na(esed)) %>%
  group_by(Stress, inducedCategory) %>%
  summarise(n = n(),
            mean_esed = ave(esed),
            sd_esed = sd(esed),
...
#plotting means +/-standard error
p_stressSpecificEscape_etohAzide <- ggplot(data = df_etohAzideStress_long %>% filter(Stress %in% c('7.5% Ethanol','0.8% Azide')),
       aes(y = esed, x = inducedCategory, fill = inducedCategory)) +
  geom_jitter(shape=16, height = 0, alpha = 0.03) +
  geom_hline(yintercept = 0, linetype = 'dashed') +
  geom_col(data = df_etohAzideStress_summary,
           aes(y = mean_esed),
           color = 'black',
           alpha = 0.8) +
  geom_errorbar(data = df_etohAzideStress_summary,
           aes(y = mean_esed, ymin = mean_esed - se_esed, ymax = mean_esed + se_esed),
           color = 'black',
           alpha = 1,
           width = 0.2) +
  ...
  scale_y_continuous(name = 'Escape score (stress)',
                     limits = c(-2,3)) +
  scale_fill_manual(values = c('grey','#A155B9','forestgreen','brown')) +
  facet_grid(cols = vars(Stress)) +
  theme(legend.position = 'none')
p_stressSpecificEscape_etohAzide

```

# QC - taking closer look at spread esed in different stresses 
Rationalizing as d/t only normalizing to spread of mock. Those variance in treatment can be different between different stresses.

```{r}
ggplot(data = df_Zsup_mean, aes(x = interaction(Treatment,Temperature), y = esc.mean)) +
         geom_jitter(shape = 16,height = 0, alpha = 0.002) +
         stat_summary(fun.y = mean, fun.ymin = function(x) mean(x) - sd(x), fun.ymax = function(x) mean(x) + sd(x), 
                      geom = 'errorbar', width = .4) +
  facet_grid(rows = vars(Treatment_group)) +
  ...
  mutate(UTR5=if_else(str_detect(Group,"HSP26"),"HSP26","PMU1"),
         Reporter_age = if_else(str_detect(Group,"New"),"New",
                                if_else(str_detect(Group,"Old"),"Old",
                                        "native"))) %>% 
  mutate(Reporter_age = factor(Reporter_age, levels = c("Old", "New"))) %>% mutate(Probe =factor(Probe, levels = c("PMU1", "HSP26", "YONL", "BEM2", "PGK1")))
df_stats_SPFC_native <- df %>%
  filter(Skip == FALSE) %>% select(-Skip) %>%
  ungroup %>%
  pivot_longer(cols = c(SP.FC,Occ_odds,SP,Occ_odds.FC,Occupancy_corr),
               names_to = "key", values_to = "value") %>%
  filter(key == "SP.FC") %>%
  filter(Probe %in% c("HSP26","PMU1")) %>%
  group_by(Temperature,key) %>%
  nest %>%
  mutate(t_test = map(data, function(df){
    t.test(df$value ~ df$Probe, 
           alternative = "two.sided"
    ) %>% broom::tidy()
  })) %>%
  select(-data) %>%
  unnest(t_test) %>%
  select(Temperature, key, p.value) %>%
  mutate(p.format = if_else(p.value < 0.01, formatC(p.value, format = "e", digits = 1),
                            format(p.value, digits = 2)))
p_native_SPFC_stats <- df_stats_SPFC_native %>%
  filter(Temperature != "30C") %>%
  mutate(xmin = case_when(Temperature == "42C" ~ 1.75,
                          Temperature == "46C" ~ 2.75),
         xmax = case_when(Temperature == "42C" ~ 2.25,
                          Temperature == "46C" ~ 3.25),
         y_position = case_when(Temperature == "42C" ~ (-0.75),
                          Temperature == "46C" ~ (-2))
  )
dodge_wid <- 0.9
SP.FC.breaks <- c(-2,-1,0)
SP.FC.labels <- c(expression(10^{-2}), expression(10^{-1}),
                  expression(10^{0}))
...
p_native_SPFC <- ggplot(df %>% filter(Probe!="YONL",Probe%in%c("HSP26","PMU1")),
  ...
  # geom_signif(
  #   data = p_native_SPFC_stats,
  #   aes(xmin = xmin, xmax = xmax, y_position = y_position, annotations = p.format),
  #   textsize = 6*0.35,
  #   size = 0.5,
  #   color = "black",
  #   tip_length = (-0.02),
...
ggsave(file.path(github_dir,"figures","Figure4/reporters_SPFC.pdf"),
       egg::set_panel_size(p=native_rep_comb_plot,width=unit(25,"mm"),height=unit(30,"mm")),
       width=100,height=100,units="mm",bg="transparent")

ggsave(file.path(github_dir,"figures","Figure4/native_SPFC.pdf"),
       egg::set_panel_size(p=p_native_SPFC,width=unit(25,"mm"),height=unit(30,"mm")),
       width=100,height=100,units="mm",bg="transparent")
```
## reporter stats
```{r}
...
# anova
df_YONL <- df %>%
  filter(Probe == "YONL", Temperature != "30C")
full_model_SPFC <- aov(SP.FC ~ Temperature * Reporter_age * UTR5, data = df_YONL)
summary(full_model_SPFC)
full_model_Occ <- aov(Occ_odds ~ Reporter_age * UTR5,
                      data = df_YONL %>% filter(Temperature == "42C"))
summary(full_model_Occ)
...
df_reporter_pSup_total <- df_42C_pSup %>% rename("Abund.norm.PGK1" = "mixSum_norm_PGK1") %>%
  select(Probe,UTR5,Reporter_age,Biorep,Temperature,Abund.norm.PGK1,Experiment_group) %>%
  bind_rows(df_46C_pSup %>%
             select(Probe,UTR5,Reporter_age,Biorep,Temperature,Abund.norm.PGK1,Experiment_group))
df_total <- df_reporter_pSup_total %>%
  mutate(Group=case_when(Probe!="YONL"~"native",
                         UTR5=="PMU1"&Reporter_age=="New"~"PMU1_New",
                         UTR5=="PMU1"&Reporter_age=="Old"~"PMU1_Old",
                         UTR5=="HSP26"&Reporter_age=="New"~"HSP26_New",
...
df_total_sum <- df_total %>%
  group_by(Temperature,Experiment_group,Group,Probe,UTR5,Reporter_age) %>%
  summarise(Abund.norm.PGK1.mean = exp(mean(log(Abund.norm.PGK1))))

df_total_sum_native <- df_total %>%
  group_by(Temperature,Experiment_group,Group,Probe) %>%
  summarise(Abund.norm.PGK1.mean = exp(mean(log(Abund.norm.PGK1))))

p_rep_total_42C <- ggplot(df_total %>% filter(Group != "native", Probe == "YONL", Experiment_group == "42C"),
                      aes(x = Temperature, y = Abund.norm.PGK1, color = Reporter_age)) +
  geom_point(size = 1.5, alpha = 0.8, shape = 16,
           position = position_dodge(width = 0.9)) +
  geom_col(data = df_total_sum %>% filter(Group != "native", Probe == "YONL", Experiment_group == "42C"),
           aes(x = Temperature, y = Abund.norm.PGK1.mean, fill = Reporter_age),
...
p_rep_total_46C <- ggplot(df_total %>% filter(Group != "native", Probe == "YONL", Experiment_group == "46C"),
                      aes(x = Temperature, y = Abund.norm.PGK1, color = Reporter_age)) +
  geom_point(size = 1.5, alpha = 0.8, shape = 16,
           position = position_dodge(width = 0.9)) +
  geom_col(data = df_total_sum %>% filter(Group != "native", Probe == "YONL", Experiment_group == "46C"),
           aes(x = Temperature, y = Abund.norm.PGK1.mean, fill = Reporter_age),
           position = position_dodge(width = 0.9), alpha = 0.5) +
  geom_point(size = 1.5, alpha = 0.8, shape = 16,
             position = position_dodge(width = 0.9)) +
...
p_native_total_42C <- ggplot(df_total %>% filter(Probe == "PMU1" | Probe == "HSP26", Experiment_group == "42C") %>% mutate(Probe = factor(Probe, levels = c("PMU1", "HSP26"))),
                      aes(x = Temperature, y = Abund.norm.PGK1, color = Probe)) +
  geom_point(size = 1.5, alpha = 0.8, shape = 16,
           position = position_dodge(width = 0.9)) +
  geom_col(data = df_total_sum_native %>% filter(Probe == "PMU1" | Probe == "HSP26", Experiment_group == "42C") %>% mutate(Probe = factor(Probe, levels = c("PMU1", "HSP26"))),
           aes(x = Temperature, y = Abund.norm.PGK1.mean, fill = Probe),
           position = position_dodge(width = 0.9), alpha = 0.5) +
  geom_hline(yintercept = 1, linetype = "dashed") +
  scale_fill_manual(values=c("PMU1"="green4","HSP26"="orange2"))+
  scale_color_manual(values=c("PMU1"="green4","HSP26"="orange2"))+
...
p_native_total_46C <- ggplot(df_total %>% filter(Probe == "PMU1" | Probe == "HSP26", Experiment_group == "46C") %>% mutate(Probe = factor(Probe, levels = c("PMU1", "HSP26"))),
                      aes(x = Temperature, y = Abund.norm.PGK1, color = Probe)) +
  geom_point(size = 1.5, alpha = 0.8, shape = 16,
           position = position_dodge(width = 0.9)) +
  geom_col(data = df_total_sum_native %>% filter(Probe == "PMU1" | Probe == "HSP26", Experiment_group == "46C") %>% mutate(Probe = factor(Probe, levels = c("PMU1", "HSP26"))),
           aes(x = Temperature, y = Abund.norm.PGK1.mean, fill = Probe),
           position = position_dodge(width = 0.9), alpha = 0.5) +
  geom_hline(yintercept = 1, linetype = "dashed") +
  scale_fill_manual(values=c("PMU1"="green4","HSP26"="orange2"))+
...
ggsave(file.path(github_dir,"figures","Figure4/reporters_total_42C.pdf"),
       egg::set_panel_size(p=p_rep_total_42C,width=unit(25,"mm"),height=unit(25,"mm")),
       width=100,height=100,units="mm",bg="transparent")
ggsave(file.path(github_dir,"figures","Figure4/reporters_total_46C.pdf"),
       egg::set_panel_size(p=p_rep_total_46C,width=unit(25,"mm"),height=unit(25,"mm")),
       width=100,height=100,units="mm",bg="transparent")
```
...
plot_fc_top100_no_margdist_nocross <- function(df,xvar,yvar,
  ...
  p_scatter <- ggplot(df,
    ...
    geom_vline(xintercept = xint,
               linetype = 'dashed') +
    ggrastr::rasterize(dpi = 600, geom_point(data=df%>%filter(gene%!in%genes_to_lab,top100==FALSE),alpha=0.6,shape=16,size=1)) +
    ggrastr::rasterize(dpi = 600, geom_point(data=df%>%filter(gene%!in%genes_to_lab,top100==TRUE),alpha=0.5,shape=16,size=1)) +
    ggrastr::rasterize(dpi = 600, geom_point(data=df%>%filter(gene%in%genes_to_lab,top100==FALSE),alpha=1,shape=16,size=1.5,color="black")) +
    ggrastr::rasterize(dpi = 600, geom_point(data=df%>%filter(gene%in%genes_to_lab,top100==TRUE),alpha=1,shape=16,size=1.5)) +
    guides(color="none")+
    scale_color_manual(values=c("TRUE"=col, "FALSE"="grey"))+
...
ggsave(file.path(github_dir,"figures","Figure4/esc_byTE_TPMup_HS_EtOH.pdf"),
       egg::set_panel_size(p=p_esc_byTPMbyTE_up,
                           width=unit(30,"mm"),height=unit(35,"mm")),bg="transparent")
```

# Figure 5
## E: rSed vs Occ

```{r fig5, dependson="rnafold"}
set.seed(100)
df_bg <- df_Zsup_mean |> filter(Treatment_group=="hairpinReporters") |>
  group_by(ORF) |>
...
#df_bg <- df_Zsup_mean |> filter(Treatment_group=="EW_TSPP", Temperature=='30C') |>
#  rename(TPM.mean = Total.TPM.mean)
...
df = df_bg |> full_join(df_30C_occ,by="ORF") |>
...
energy_color_scale = c("least" = paste(RColorBrewer::brewer.pal(12,"Paired")[4],'AA',sep=''),
                              "other" = "#aaaaaaaa",
                              "most" = paste(RColorBrewer::brewer.pal(12,"Paired")[10],'AA',sep=''))
dsum = df |> filter(EnergyBin %in% c('least','most'))
dstat = dsum |> group_by(EnergyBin) |> 
  summarise(mrsed=mean(RelSed.mean,na.rm=T), sdrsed=sd(RelSed.mean,na.rm=T), n=n(),
                                                 mocc=mean(Occ.odds.mean,na.rm=T), sdocc=sd(Occ.odds.mean,na.rm=T)) |>
  mutate(sersed=sdrsed/sqrt(n), seocc=sdocc/sqrt(n))
p_Occ = ggplot(df,
  ...
  scale_fill_manual(values=energy_color_scale)+
  #stat_ellipse(data=df |> filter(EnergyBin %in% c('least','most')), aes(fill=EnergyBin), alpha=0.3, level=0.95, type='norm') +
...
p_Occ_vs_rSed = ggplot(df,
       aes(x=Occ.odds.mean,y=RelSed.mean))+
  geom_hline(yintercept=0,linetype="dotted")+
  geom_vline(xintercept=median(df$Occ.odds.mean,na.rm=T),linetype="dotted")+
  ggrastr::rasterize(dpi = 600, geom_point(alpha=0.3, shape = 16, size = 1, color="black")) +
  scale_x_log10nice()+
  coord_cartesian(ylim=c(-2.1,3.5)) +
  geom_point(data=df |> filter(gene%in%genes_to_label),size=2,color="red")+
  geom_text_repel(data=df |> filter(gene%in%genes_to_label),size=3,aes(label=gene))+
  labs(x="",y="")+
  theme(legend.position="none")
p_Occ_vs_rSed

# Isolate transcripts with high TPM
df_high = df |> filter(TPM.mean>1000) # median
p_Occ_vs_rSed_tpm = ggplot(df_high,
       aes(x=Occ.odds.mean,y=RelSed.mean))+
  geom_hline(yintercept=0,linetype="dotted")+
  geom_vline(xintercept=median(df_high$Occ.odds.mean,na.rm=T),linetype="dotted")+
  ggrastr::rasterize(dpi = 600, geom_point(alpha=0.3, shape = 16, size = 1, color="black")) +
  scale_x_log10nice()+
  coord_cartesian(ylim=c(-2.1,3.5)) +
  geom_point(data=df_high |> filter(gene%in%genes_to_label),size=2,color="red")+
  geom_text_repel(data=df_high |> filter(gene%in%genes_to_label),size=3,aes(label=gene))+
  labs(x="",y="")+
  theme(legend.position="none")
p_Occ_vs_rSed_tpm
...
p_zsup_box = ggplot(df |> filter(!is.na(EnergyBin)),
  ...
  scale_fill_manual(values=energy_color_scale) + #, function(x){if (!x=='grey50'){paste(x,"AA",sep='')}else{x}})) +
  scale_color_manual(values=energy_color_scale) +
  scale_alpha_manual(values=c("least"=0.6, "other"=0.6, "most"=0.6)) +
  coord_cartesian(ylim=c(-2.1,3.5)) +
  geom_violin(draw_quantiles=c(0.5), size=0.5, color="black", trim=F) +
    theme(legend.position="none") +
  ggsignif::geom_signif(na.rm=T,
                        map_signif_level=T,
                        test="wilcox.test",
...
ggsave("../figures/Figure5/Zsup_Occ_left.pdf",
       ...
       width=75,height=75,units="mm",bg="transparent")
ggsave("../figures/Figure5/rSed-vs-occ.pdf",
       egg::set_panel_size(p=p_Occ_vs_rSed,width=unit(50,"mm"),height=unit(50,"mm")),
       width=75,height=75,units="mm",bg="transparent")
...
p_Occ_RelSed <- ggplot(df,
  ...
  geom_hline(yintercept=0,linetype="dotted")+
  geom_vline(xintercept=median(df$Occ.odds.mean,na.rm=T),linetype="dotted")+
  ggrastr::rasterize(dpi = 600, geom_point(data=df%>%filter(EnergyBin=="other"),alpha=0.5, shape = 16, size = 0.5, color = "grey")) +
  ggrastr::rasterize(dpi = 600, geom_point(data=df%>%filter(EnergyBin!="other"),alpha=0.75,aes(color=EnergyBin), shape = 16, size = 0.5)) +
  scale_color_manual(values=energy_color_scale)+
  #scale_x_log10nice()+#coord_cartesian(xlim=c(0.2,50))+
  scale_x_log10nice()+
  geom_text(data=df_corr %>% filter(term=="Occ.odds.mean"),
...
p_RelSed_box <- ggplot(df %>% filter(!is.na(EnergyBin)),
         aes(x=EnergyBin,y=RelSed.mean),fill=EnergyBin)+
    ggrastr::rasterize(dpi = 600, geom_point(position=position_jitterdodge(jitter.width=1.5),
               aes(color=EnergyBin,alpha=EnergyBin), shape =16, size = 0.5)) +
  scale_color_manual(values=energy_color_scale)+
...
p_TPM_TE <- ggplot(df,
  ...
  ggrastr::rasterize(dpi = 600, geom_point(alpha=0.3, size = 0.5, shape = 16, color = "black")) +
  ggrastr::rasterize(dpi = 600, geom_point(data=df %>% filter(gene%in%genes_to_label),size=2,color="red")) +
  geom_text_repel(data=df%>% filter(gene %in% genes_to_label),size=4.2,aes(label=gene_label),max.overlaps = 10000,min.segment.length = 0, segment.color = NA)+
  labs(x="mRNA Abundance",y="Translation\nEfficiency")
p_TPM_TE

df_unstress = df_Zsup_mean |> filter(Treatment_group=='EW_TSPP', Temperature=='30C')
p_RelSed_box <- ggplot(df_unstress,
                  aes(x=1,y=RelSed.mean))+
  ggrastr::rasterize(dpi = 600, geom_point(data=df_unstress%>%filter(gene%!in%genes_to_label),position=position_jitter(width=0.43),alpha=0.03, size = 0.09)) +
  #geom_boxplot(fill=NA,outlier.shape=NA)+
  geom_violin(fill="grey70", alpha = 0.6, draw_quantiles = c(0.5))+
  ggrastr::rasterize(dpi = 600, geom_point(data=df_unstress%>%filter(gene%in%genes_to_label),alpha=1,color="red",size=1)) +
  #geom_text_repel(data=df%>%filter(gene=="GCN4"),size=4.2,aes(label=gene_label),max.overlaps = 10000,min.segment.length = 0,nudge_y=2.5,nudge_x=0.25)+
  #geom_text_repel(data=df%>%filter(gene=="HAC1"),size=4.2,aes(label=gene_label),max.overlaps = 10000,min.segment.length = 0,nudge_y=-2.5 ,nudge_x=0.25)+
  labs(x="",y="")+
  theme(axis.title.x=element_blank(),  # Removes x-axis label
...
df_hairpins <- df_Zsup_mean %>% filter(Treatment_group=="hairpinReporters") %>%
  ungroup %>%
  mutate(pSup.unstructured.sd = sd(logodds(pSup.mean[Strain == "yHG010"]) - logodds(pSup.treatment.window.mean.mean[Strain == "yHG010"]), na.rm=T)) %>%
  group_by(ORF) %>%
  filter(length(ORF[Strain == "yHG010"]) == 1) %>%
  mutate(deltaSed.unstructured = (logodds(pSup.mean[Strain == "yHG010"]) - logodds(pSup.mean)) / pSup.unstructured.sd)
...
df <- df_hairpins %>%
  ...
  filter(Strain%in%df_reporter_TE$Strain,Treated==FALSE,Temperature=="30C") %>%
  filter(ORF=="strong_Clover") %>%
  select(ORF,Strain,Treatment_group,Treatment,Treated,Temperature,deltaSed.unstructured) %>%
  group_by(ORF,Strain) %>%
  summarise(n = length(deltaSed.unstructured),
            sed.se = sd(deltaSed.unstructured)/sqrt(length(deltaSed.unstructured)),
            sed.mean = mean(deltaSed.unstructured)) %>%
  left_join(df_reporter_TE,by="Strain") %>%
  mutate(Strain=factor(Strain,levels=c("yHG010","yHG005","yHG006","yHG007","yHG008","yHG027","yHG026"))) %>%
  arrange(Strain)
...
strain_tibble <- tribble(~"Strain",~"Desc",~"color",~"shape",
                         "yHG010","unstructured","black",1,
                         "yHG005","weakest","#AC97D0",16,
                         "yHG006","weak","#9668D5",16,
                         "yHG007","medium","#6B41AA",16,
                         "yHG008","strong","#411C80",16,
                         "yHG026","uORF",RColorBrewer::brewer.pal(8,"Dark2")[4],15,
                        #"yHG027","uORF_control",RColorBrewer::brewer.pal(8,"Dark2")[4],0)
                        "yHG027","uORF_control","black",0)
strain_colors <- setNames(strain_tibble$color,strain_tibble$Strain)
strain_shapes <- setNames(strain_tibble$shape,strain_tibble$Strain)
p_reps <- ggplot(df %>%arrange(desc(TE_norm)),
       aes(x=TE_norm,y=sed.mean))+
  geom_hline(yintercept = 0, linetype = "dotted") +
  geom_errorbar(aes(x=TE_norm,ymin=sed.mean-sed.se,ymax=sed.mean+sed.se),width=0.5,size=0.5)+
  ggrastr::rasterize(dpi = 600, geom_point(aes(color=Strain,shape=Strain),size=3.1)) +
  scale_y_continuous(breaks=c(0,2,4))+
  scale_color_manual(values=strain_colors)+
  scale_shape_manual(values=strain_shapes)+
  scale_x_log10nice()+
  coord_cartesian(xlim=c(0.001,1.6), ylim = c(-1,4))+
  theme(legend.position="none")+
  labs(x="TE (norm. to unstructured)",y="∆Sed (SDs)\n(relative to unstructured reporter strain)")
p_reps


ylims <- ggplot_build(p_reps)$layout$panel_params[[1]]$y.range

p_dens <- ggplot(df_hairpins %>% filter(Strain != "yHG010"),aes(x=deltaSed.unstructured))+
  geom_density(fill = "grey80")+
  theme_void()+
  coord_flip(xlim=c(ylims[1],ylims[2]), expand = F)
p_dens
p_reps + p_dens+ plot_layout(widths = c(2, 1))

ggsave(file.path(github_dir,"figures/Figure5/TE_hairpins_deltaSed.pdf"),
       egg::set_panel_size(p=p_reps+labs(x="",y=""),width=unit(30,"mm"),height=unit(30,"mm")),
       width=75,height=75,units="mm",bg="transparent")
ggsave(file.path(github_dir,"figures/Figure5/TE_dens_hairpins_deltaSed.pdf"),
       egg::set_panel_size(p=p_dens,width=unit(8,"mm"),height=unit(30,"mm")),
       width=75,height=75,units="mm",bg="transparent")
```
...
p_gel <- ggplot(df,
 ...
 scale_color_manual(values=c("Sup"= "red3",
                              "Pel"="grey60"))+
  scale_fill_manual(values=c("Sup"="red3",
                              "Pel"="grey60"))+
...
## hac1 violin
## Figure S5B, escape of HAC1 in stress, transcriptional induction.
```{r}
min = (-6)
max = 6
df = df_Zsup_mean %>%
...
names(stress.cols)[names(stress.cols) == "DTT.reddishpurple"] <- "DTT"

p_dtt = ggplot(df,
                aes(x=lab,y=esc.mean)) +
  ggrastr::rasterize(dpi = 600, geom_point(data=df%>%filter(gene!="HAC1"),aes(y=esc_compress),
             position=position_jitter(width=0.4),alpha=0.1,shape=16,
             size = 0.2, color = "grey50"))+
  geom_violin(alpha=1,aes(fill = lab, width = 1))+
  scale_fill_manual(values = c("42°C" = stress.cols[["42C"]],
                    "46°C" = stress.cols[["46C"]],
                    "DTT" = stress.cols[["DTT"]])) +
  geom_violin(draw_quantiles=c(0.5),fill=NA, color ="black", width = 1)+
  coord_cartesian(ylim=c(-4.75,4.75))+
  scale_y_continuous(breaks=c(-4,0,4))+
  theme(axis.ticks.x=element_blank())+
  geom_point(data=df%>%filter(gene=="HAC1"),size=2,shape=21,fill="red3",alpha=1, color = "black", stroke = 0.5) +
  no.legend
p_dtt
ggsave(file.path(github_dir,"figures","Figure5","DTT_esc.pdf"),
                 p_dtt+labs(x="",y=""),
       width=42.55,height=42.55,units="mm",bg="transparent")
```
## rSed vs. occupancy
# Fig. S5A
```{r rsed_vs_occ, dependson="setup"}
target_genes = c('HAC1','GCN4')

df = df_unstress |> left_join(df_30C_occ)
with(df, cor.test(Occ.odds.mean, RelSed.mean, method='s'))
g_occ_rsed = ggplot(df, aes(x=Occ.odds.mean, y=RelSed.mean)) + 
  ggrastr::rasterize(dpi = 600, geom_point(alpha=0.2, size = 0.5, shape = 16)) +
  scale_x_log10nice("Ribosome occupancy (odds)") + 
  geom_point(data=df |> filter(gene %in% target_genes), size=2, colour=redcol, alpha=1, shape=16) +
  geom_text_repel(data=df |> filter(gene %in% target_genes), aes(label=gene)) +
  ylab("Relative sedimentation (rSed)")
g_occ_rsed
ggsave(file.path(github_dir,"figures","Figure5","Occ-vs-rSed.pdf"),
                g_occ_rsed,
       width=50,height=50,units="mm",bg="transparent")

```

## Transcriptional induction of HAC1
# Fig. S5A
```{r dependson="setup"}
target_genes = c('HAC1','SSA4')

dfdtt = df_Zsup_Poly_minfilt |> filter(Stress_group=='DTT 10mM', Treatment=='DTT_10mM')

df = df_Zsup_mean |> filter(Treatment_group %in% c('EW_TSPP'), !(Treatment %in%c('42CR'))) |> select(ORF, gene, Treatment, Temperature, Total.TPM.mean)
#df = bind_rows(df_Zsup_mean |> filter(Treatment_group %in% c('EW_TSPP'), !(Treatment %in%c('42CR'))) |> select(ORF, gene, Temperature, Total.TPM.mean) |> rename(Treatment=Temperature),
#               df_Zsup_mean |> filter(Treatment_group %in% c('uprInduction')) |> select(ORF, gene, Treatment, Total.TPM.mean))

dfw = df |> pivot_wider(id_cols=c(ORF,gene), values_from=Total.TPM.mean, names_from=Temperature,names_prefix = 'tpm.') |> 
  mutate(`42C`=tpm.42C/tpm.30C, `46C`=tpm.46C/tpm.30C)
dat = dfdtt |> rename(DTT=TPM.FC.SedSeq) |> left_join(dfw |> select(ORF, `42C`,`46C`))

datfc = dat |> select(ORF, gene, DTT, `46C`,`42C`) |> pivot_longer(cols=c('42C','46C','DTT'), values_to="fc", names_to='Treatment') # |> select(-tpm.30C, -tpm.42C, -tpm.46C, -tpm.mock, -tpm.DTT_10mM)

dg_fc_heat = ggplot(datfc, aes(x=Treatment, y=fc)) + 
  ggrastr::rasterize(dpi = 600, geom_jitter(alpha=0.2, size = 0.5, shape = 16, width=0.25)) +
  geom_violin(aes(fill=Treatment), alpha=0.9) +
  scale_x_discrete(labels=c('42°C','46°C','DTT'))+
  scale_y_log2nice("mRNA fold change vs. untreated", lim=c(0.25,64)) + 
  geom_point(data=datfc |> filter(gene %in% target_genes), size=2, colour=blackcol, alpha=1, shape=16) +
  geom_label_repel(data=datfc |> filter(gene %in% target_genes), aes(label=gene), nudge_x = 0.3, min.segment.length = 0.1, size=3, alpha=0.8, label.size=0) +
  scale_fill_manual(values=stress.cols) +
  xlab("Treatment") +
  no.legend
dg_fc_heat
ggsave(file.path(github_dir,"figures","FigureS5","tpm-fc-HAC1.pdf"),
                dg_fc_heat,
       width=50,height=50,units="mm",bg="transparent")
...
chx_sg_per_cell_plot <- ggplot(data = chx_hs_master_merge, aes(x = Temperature, y = Cell_Parent_Children_stress_granule_Count, fill=Temperature,group=interaction(Temperature,CHX))) +
  ggrastr::rasterize(geom_point(alpha=0.2,position=position_jitterdodge(jitter.width=0.2,dodge.width = 0.75),color="grey50", size = 0.2, shape = 16),dpi=600)+  
...
```
eIF3b depletion plots (Fig. 7B)
```{r}
aid_hs_sg <- read_csv(file.path(github_dir,"data_processed/sg_quant/aid_hs.csv"))
aid_hs_sg$Depletion = factor(aid_hs_sg$Depletion, levels=c('Mock','eIF3b depletion'))
# Check statistics:
# aid_hs_sg |> group_by(Temperature, Depletion) |> summarise(SG.cell=median(SG_per_cell_compressed), SG.cell2=median(Cell_Parent_Children_stress_granule_Count))

aid_sg_per_cell_plot <-ggplot(data = aid_hs_sg , aes(x = Temperature, y = Cell_Parent_Children_stress_granule_Count, fill=Temperature, group=interaction(Depletion, Temperature))) +
  ggrastr::rasterize(geom_point(aes(x = Temperature, y = SG_per_cell_compressed),alpha=0.2,position=position_jitterdodge(jitter.width=0.2,dodge.width = 0.75),color="grey50", size = 0.2, shape = 16), dpi=600) +
...
## noise relsed hairpin replicates
```{r}
df <- df_Zsup_mean %>%
  filter(Treatment_group == "hairpinReporters",
         pSup.mean>0,SP.mean>0,!is.infinite(pSup.mean),!is.infinite(SP.mean)) %>%
  filter(classification=="Verified") %>%
  group_by(ORF,gene,LengthTxEst) %>%
  summarise(pSup.mean = invlogodds(mean(logodds(pSup.mean))),
            SP.mean.all = exp(mean(log(SP.mean))),
            SP.sem.all  = exp(sd(log(SP.mean))/sqrt(n())),
            pSup.windowmean = invlogodds(mean(logodds(pSup.treatment.window.mean.mean))),
            RelSed.mean = mean(RelSed.mean)) %>%
  ungroup %>%
  mutate(lo.pSup.hairpin.sd = sd(logodds(pSup.mean) - logodds(pSup.windowmean), na.rm=T))
# not bad set.seed(1)
set.seed(3)
which_genes <- df_Zsup_mean %>%
  filter(Treatment_group == "hairpinReporters",
  ...
  filter(!is.na(gene),classification=="Verified",
         LengthTxEst < 6e3,LengthTxEst > 300) %>%
  ungroup %>%
  mutate(RelSed_group = if_else(RelSed.mean>=0,"RelSed.up","RelSed.down")) %>%
  mutate(length_bin = cut_interval(log(LengthTxEst),n=6)) %>%
  group_by(length_bin,RelSed_group) %>%
  nest %>%
  mutate(sampled = map(data,function(df) {
    return(tibble(sampled_genes = sample(df$gene,size=1)))
  })) %>%
...
p <- ggplot(df,
            aes(x=LengthTxEst,y=SP.mean.all))+
  ggrastr::rasterize(dpi = 600, geom_point(alpha=0.3,shape=16,size=0.5,color="grey60")) +
  geom_line(aes(y=odds(pSup.windowmean)),color="black",size=0.5,alpha=0.5,linetype=1,linewidth=0.5)+
  geom_line(aes(y=exp(logodds(pSup.windowmean) - lo.pSup.hairpin.sd)),color=okabe_ito_colors[4],size=1,linewidth=0.5, linetype=1)+
  geom_line(aes(y=exp(logodds(pSup.windowmean) + lo.pSup.hairpin.sd)),color=okabe_ito_colors[4],size=1, linewidth=0.5, linetype=1)+
  scale_x_log10nice()+ scale_y_log10nice()+
  coord_cartesian(ylim=c(0.5,40))+
  geom_point(data=df %>% filter(gene %in% c(genes_to_highlight)),
...
ggsave("../figures/Supplemental/hairpin_relsed_30C_error.pdf",
        egg::set_panel_size(p=p,
                            width=unit(50,"mm"),height=unit(50,"mm")),
       width=75,height=75,units="mm",bg = "transparent",dpi=600)
```
...
df <- df_Zsup_mean %>%
  ...
  t_result_hac1 <- t.test(df %>% filter(gene == "HAC1") %>% pull(RelSed.mean), df %>% filter(gene != "HAC1") %>% pull(RelSed.mean), 
                    alternative = "less",
                    var.equal = FALSE)
  t_result_hac1
    t_result_gcn4 <- t.test(df %>% filter(gene == "GCN4") %>% pull(RelSed.mean), df %>% filter(gene != "HAC1") %>% pull(RelSed.mean), 
                    alternative = "less",
                    var.equal = FALSE)
    t_result_gcn4
```
...
p_SP <- ggplot(df %>% filter(!is.infinite(SP.mean),SP.mean>0),
  ...
  geom_segment(data = data.frame(x = hac1_length, y = hac1_30C_SP, xend = hac1_30C_equiv_length, yend = hac1_30C_SP),
               aes(x=x,y=y,xend=xend,yend=yend),
               linetype="dotted",color="black") +
  geom_segment(data = data.frame(x = hac1_30C_equiv_length, y = 1e-10, xend = hac1_30C_equiv_length, yend = hac1_30C_SP),
               aes(x=x,y=y,xend=xend,yend=yend),
               linetype="dotted",color="black") +
...
ggsave("../figures/Supplemental/HAC1_equiv_length.pdf",
       egg::set_panel_size(p=p_SP,
                           width=unit(40,"mm"),height=unit(40,"mm")),units="mm",bg = "transparent")
```
...
upper<-4 
lower<-(-3)
df <- df_Zsup_Poly_minfilt %>% filter(Stress_group == "Heat Shock") %>%
...
p_esc_FC <- ggplot(df,
       aes(x=TPM.FC.SedSeq,y=esc.mean, color = target))+
  geom_hline(yintercept=0,linetype="dotted")+
  ggrastr::rasterize(dpi = 600, geom_point(data = df %>% filter(target == "other"),shape=16,alpha=0.5, size = 0.5)) +
  ggrastr::rasterize(dpi = 600, geom_point(data = df %>% filter(target == "MSN2/4 targets"),shape=16,alpha=0.5, size = 1)) +
  ggrastr::rasterize(dpi = 600, geom_point(data = df %>% filter(target == "HSF1 targets"),shape=16,alpha=0.5, size = 1)) +
  scale_color_manual(values=target.cols)+
  scale_x_log10nice() +
  ...
  no.legend
p_esc_FC
ggsave("../figures/Supplemental/TPM_FC_esc_hsf1_msn24.pdf",
       egg::set_panel_size(p=p_esc_FC,width=unit(30,"mm"),height=unit(30,"mm")),
...
wilcox_result <- df %>%
  group_by(Temperature) %>%
  nest %>%
  mutate(wilcox_hsf1 = map(data,function(df){
    wilcox.test(esc.mean ~ target, data = df, 
                             subset = (target %in% c("other", "HSF1 targets")),
                             exact = FALSE
    )
  }),
  wilcox_msn24 = map(data,function(df){
    wilcox.test(esc.mean ~ target, data = df, 
                             subset = (target %in% c("other", "MSN2/4 targets")),
                             exact = FALSE
    )
  }))
print(wilcox_result$wilcox_hsf1)
print(wilcox_result$wilcox_msn24)
```
...
ggsave("../figures/Supplemental/khong_induced.pdf",
        egg::set_panel_size(p=khong_induced,width=unit(40,"mm"),height=unit(40,"mm")),
       width=100,height=100,units="mm",bg="transparent")
khong_induced
```
...
df <- df_Zsup_mean %>%
  ...
  mutate(Stress_label=factor(Stress_label,
                             levels=c("mock","42C","46C","Azide 0.5%","Azide 0.8%",
                                      "Ethanol 5%","Ethanol 7.5%","Ethanol 10%","Ethanol 15%"))) %>%
  filter(Stress_label != "mock") %>%
  mutate(sed.compress = if_else(sed.mean>maxSed,maxSed,
                                   if_else(sed.mean<minSed,minSed,sed.mean)))
stress_labels <- c("mock"="mock",
                   "42C"="42°C",
                   "46C"="46°C",
                   "Azide 0.5%"="0.5%",
...
p_length_hs <- ggplot(df %>% filter(Stress_label %in% c("42C", "46C")),
                   aes(x=LengthTxEst,y=sed.mean,color=Stress_label))+
  geom_hline(yintercept=0,linetype="dotted")+
  ggrastr::rasterize(dpi = 600, geom_point(aes(y=sed.compress),alpha=0.2, shape = 16, size = 0.3)) +
  scale_color_manual(values=stress.cols)+
  facet_grid(.~Stress_label,
             labeller=labeller(Stress_label=stress_labels))+
  scale_x_log10nice()+
  geom_smooth(method="lm",se=F,na.rm=T,color="black")+
  coord_cartesian(xlim=c(200,20000), ylim = c(minSed,maxSed)) +
  theme(panel.spacing.x = grid::unit(0, "mm"),
        strip.placement = "outside",
        strip.background = element_blank(),
        aspect.ratio = 1)+
...
df <- df_Zsup_Poly_minfilt %>% 
  ...
  mutate(Induction_cat = case_when(TPM.FC.SedSeq < quantile(TPM.FC.SedSeq,0.1) ~ "down",
                                   ...
                                   TRUE ~ "other")) %>%
  mutate(esc.compress = if_else(esc.mean>maxEsc,maxEsc,
                                   if_else(esc.mean<minEsc,minEsc,esc.mean))) %>%
  mutate(TE_cat = case_when(FC.vs.ctrl_TE < quantile(FC.vs.ctrl_TE,0.1) ~ "down",
                                   FC.vs.ctrl_TE > quantile(FC.vs.ctrl_TE,0.9) ~ "up",
                                   TRUE ~ "other"))

df_wilcox_esc_induct <- df %>%
  group_by(Stress_group,Stress_label) %>%
  nest %>%
  mutate(p_value = map(data,function(df){
    pairwise_results <- pairwise.wilcox.test(df$esc.mean, df$Induction_cat, p.adjust.method = "bonferroni")
    broom::tidy(pairwise_results)
  })) %>%
  select(-data) %>%
  unnest(p_value) %>%
...
annotation_df_esc_induct <- df_wilcox_esc_induct %>%
  add_column(x_start = c(1,2,1,2,1,2,1,2),
             x_end = c(2,3,2,3,2,3,2,3),
             y_pos = rep(3,8)) %>%
  mutate(id = row_number())
p_box_esc_induct <- ggplot(df,
                aes(x=Induction_cat,y=esc.mean,fill=Stress_label))+
  geom_hline(yintercept=0,linetype="dotted")+
  ggrastr::rasterize(dpi = 600, geom_point(aes(y=esc.compress), shape=16,alpha=0.5,size=0.5,color="grey50",
             position=position_jitter(width=0.2,seed=1))) +
  ...
  scale_fill_manual(values=stress.cols)+
  labs(x="",y="")+
  coord_cartesian(ylim=c(minEsc,maxEsc))+
  scale_y_continuous(breaks=c(-3,0,3))+
  guides(fill="none")+
  ggsignif::geom_signif(
    manual = T,
    data = annotation_df_esc_induct,
    aes(xmin = x_start, xmax = x_end, annotations = p_label, y_position = y_pos, group = id),
    textsize = 8*0.35
  )
p_box_esc_induct
ggsave("../figures/Supplemental/FC_comp_esc_induction_box.pdf",
       egg::set_panel_size(p_box_esc_induct,width=unit(30,"mm"),height=unit(30,"mm")),
       width=300,height=75,units="mm",bg="transparent",dpi=600)

df_wilcox_TE_induct <- df %>%
  group_by(Stress_group,Stress_label) %>%
  nest %>%
  mutate(p_value = map(data,function(df){
...
annotation_df_TE_induct <- df_wilcox_TE_induct %>%
  add_column(x_start = c(1,2,1,2,1,2,1,2),
             x_end = c(2,3,2,3,2,3,2,3),
             y_pos = rep(0.5,8))%>%
  mutate(id = row_number())
p_box_TE_induct <- ggplot(df,
  ...
  ggsignif::geom_signif(
    manual = T,
    data = annotation_df_TE_induct,
    aes(xmin = x_start, xmax = x_end, annotations = p_label, y_position = y_pos, group = id),
    textsize = 8*0.35
  )
p_box_TE_induct
ggsave("../figures/Supplemental/FC_comp_FCTE_induction_box.pdf",
       egg::set_panel_size(p_box_TE_induct,width=unit(30,"mm"),height=unit(30,"mm")),
       width=300,height=75,units="mm",bg="transparent",dpi=600)
...
df_wilcox_esc_TE <- df %>%
  ...
  nest %>%
  mutate(p_value = map(data,function(df){
    pairwise_results <- pairwise.wilcox.test(df$esc.mean, df$TE_cat, p.adjust.method = "bonferroni")
    broom::tidy(pairwise_results)
  })) %>%
  select(-data) %>%
  unnest(p_value) %>%
...
annotation_df_esc_TE <- df_wilcox_esc_TE %>%
  add_column(x_start = c(1,2,1,2,1,2,1,2),
             x_end = c(2,3,2,3,2,3,2,3),
             y_pos = rep(3,8)) %>%
  mutate(id = row_number())

p_box_esc_TE <- ggplot(df,
                aes(x=TE_cat,y=esc.mean,fill=Stress_label))+
  geom_hline(yintercept=0,linetype="dotted")+
  ggrastr::rasterize(dpi = 600, geom_point(aes(y = esc.compress),shape=16,alpha=0.5,size=0.5,color="grey50",
             position=position_jitter(width=0.2,seed=1))) +
  geom_boxplot(outlier.shape=NA,alpha=0.5,size=0)+
  geom_boxplot(fill=NA,outlier.shape=NA,size=0.5)+
  facet_grid(.~Stress_label,
             labeller=labeller(Stress_label=stress_labs))+
  scale_fill_manual(values=stress.cols)+
  ...
  ggsignif::geom_signif(
    manual = T,
    data = annotation_df_esc_TE,
    aes(xmin = x_start, xmax = x_end, annotations = p_label, y_position = y_pos, group = id),
    textsize = 8*0.35
  )
p_box_esc_TE
ggsave("../figures/Supplemental/FC_comp_FCTE_esc_box.pdf",
       egg::set_panel_size(p_box_esc_TE,width=unit(30,"mm"),height=unit(30,"mm")),
       width=300,height=75,units="mm",bg="transparent",dpi=600)
```
...
ggsave("../figures/Supplemental/Sucrose_rep_pPel.pdf",
...
ggsave("../figures/Supplemental/Sucrose_rep_Occ.pdf",
...
# TPM vs. occupancy, Fig. S5A
```{r occ_vs_tpm, dependson="setup"}
genes_to_label <- c("HAC1","GCN4")
df_30C_occ <- df_Occ_mean_minfilt %>%
  filter(Treatment%in%c("none","mock"),Temperature=="30C") %>%
  group_by(ORF) %>%
  summarise(Occ.odds.mean = exp(mean(log(Occ.odds.mean))))
df <- df_Zsup_mean %>% filter(Treatment_group=="hairpinReporters") %>%
  group_by(ORF,gene,Temperature,LengthTxEst) %>%
  summarise(SP.mean = exp(mean(log(SP.mean))),
            pSup.mean = mean(pSup.mean),
            TPM.mean = exp(mean(log(Total.TPM.mean))),
            RelSed.mean = mean(RelSed.mean),
            Zsup.mean = mean(Zsup.mean)) %>% full_join(df_30C_occ,by="ORF") %>%
  left_join(df_Weinberg,by="ORF") %>%
  filter(Occ.odds.mean>0) %>%
  mutate(gene_label = if_else(gene%in%genes_to_label,gene,""))
df_corr <- df %>% ungroup %>%
...
p_TPM_Occ <- ggplot(df,
  ...
  geom_point(data=df %>% filter(gene%in%genes_to_label),size=2,color="red")+
  geom_text_repel(data=df,size=4.2,aes(label=gene_label),max.overlaps = 10000,min.segment.length = 0)+
  labs(x="mRNA abundance (TPM)",y="Ribosome occupancy (odds)")
p_TPM_Occ
...
# p_3B_occ
ggsave(file.path(github_dir,"figures","Figure5","Occ-vs-TPM.pdf"),
                p_TPM_Occ,
       width=60,height=60,units="mm",bg="transparent")
#ggsave("../figures/FigureS5/HAC1_GCN4_Occupancy.pdf",egg::set_panel_size(p=p_TPM_Occ+labs(x="",y=""),width=unit(50,"mm"),height=unit(50,"mm")),bg="transparent",dpi=600)
```
...
Lsm1-7 proteins do not condense during heat shock at 46C.

```{r tiics_vs_pbs_vs_sgs, dependson='setup'}
...
gene_cat_list = list("Lsm1-7"=c('LSM1','LSM2','LSM3','LSM4','LSM5','LSM6','LSM7'),
     ...
     "SG"=c('PAB1','OLA1','PBP1','PUB1','TIF34','TIF35','PRT1','NIP1','RPG1','TIF4631','TIF4632'))
...
plot_time = ggplot(data=ps_dt_time, 
    ...
    geom_text_repel(size=4,data=subset(ps_dt_time,time==max(times)),
              aes(x=max(times)+0.1,y=psup), xlim=c(8,12), show.legend=F) +
...
ggsave(file.path(github_dir, "figures/FigureS7/FigS7_tiics_vs_pbs_vs_sgs.pdf"),
       egg::set_panel_size(plot_time, width = unit(fig.width, "mm"), height = unit(fig.width*fig.asp, "mm")),
       width = fig.width*fig.vs.panel.ratio,
       height = fig.width*fig.asp*fig.vs.panel.ratio,
       units = "mm", bg = "transparent", create.dir=TRUE)
```
## secretome analysis
```{r}
df_costa <- read_tsv(file.path(github_dir,"src/Costa_2018/NIHMS968272-supplement-Table_S3.txt"),comment="#",show_col_types = F) %>%
  rename("ORF"="gene")
df <- df_Zsup_Poly_minfilt %>% filter(Stress_group=="Heat Shock", Temperature == "30C") %>%
  group_by(ORF,gene,Temperature,LengthTxEst) %>%
  summarise(TPM.mean = exp(mean(log(TPM.SedSeq))),
            RelSed.mean = mean(RelSed.mean),
            Occ.mean = mean(Occ.mean)) %>%
  left_join(df_costa %>% rename("Secretome" = "secretome") %>% select(ORF,Secretome),by="ORF") %>%
  ungroup %>%
  mutate(TPM_bin = cut_number(TPM.mean, n = 5, labels = paste0("q",seq(1,5))),
         Occ_bin = cut_number(Occ.mean, n = 5, labels = paste0("q",seq(1,5))))
p_secretome_TPM <- ggplot(df %>% filter(!is.na(Secretome)),
                      aes(x = TPM_bin, y = RelSed.mean, fill = Secretome, group = interaction(TPM_bin,Secretome))) +
  geom_hline(yintercept=0,linetype="dotted")+
  ggrastr::rasterize(dpi = 600,
                     geom_point(shape=16, alpha=0.2, size = 0.5, color="grey50", position=position_jitterdodge(jitter.width = 0.2, dodge.width = 0.75))) +
  geom_violin(alpha = 0.5, linewidth = 0, width =0.5, position = position_dodge(width = 0.75)) +
  scale_fill_manual(values = c("no" = "grey50", "yes" = purplecol)) +
  geom_violin(fill=NA,alpha=1,draw_quantiles = c(0.5),size=0.25, width = 0.5, position = position_dodge(width = 0.75)) +
  labs(x = "mRNA abundance quintile", y = "Relative\nsedimentation at 30°C\n(rSed, SDs)")
p_secretome_TPM
ggsave(file.path(github_dir,"figures/Supplemental/compare_secretome_relsed_TPM.pdf"),
       egg::set_panel_size(p=p_secretome_TPM,width=unit(40,"mm"),height=unit(30,"mm")),bg="transparent")

wilcox.test(RelSed.mean ~ Secretome, data = df %>% filter(!is.na(Secretome)) %>% mutate(Secretome = factor(Secretome, levels = c("no","yes"))), alternative = "greater")

p_secretome_Occ <- ggplot(df %>% filter(!is.na(Secretome), !is.na(Occ_bin)),
                      aes(x = Occ_bin, y = RelSed.mean, fill = Secretome, group = interaction(Occ_bin,Secretome))) +
  geom_hline(yintercept=0,linetype="dotted")+
  ggrastr::rasterize(dpi = 600,
                     geom_point(shape=16, alpha=0.2, size = 0.5, color="grey50", position=position_jitterdodge(jitter.width = 0.2, dodge.width = 0.75))) +
  geom_violin(alpha = 0.5, linewidth = 0, width = 0.6, position = position_dodge(width = 0.75)) +
  scale_fill_manual(values = c("no" = "grey50", "yes" = purplecol)) +
  geom_violin(fill=NA,alpha=1,draw_quantiles = c(0.5),size=0.25, width = 0.6, position = position_dodge(width = 0.75)) +
  labs(x = "Ribosome occupancy quintile", y = "Relative\nsedimentation at 30°C\n(rSed, SDs)") +
  ggsignif::geom_signif(
    comparisons = list(c("no", "yes")),
    map_signif_level = TRUE
  )
p_secretome_Occ
ggsave(file.path(github_dir,"figures/Supplemental/compare_secretome_relsed_Occ.pdf"),
       egg::set_panel_size(p=p_secretome_Occ,width=unit(40,"mm"),height=unit(30,"mm")),bg="transparent")

p_secretome_Occ_facet <- ggplot(df %>% filter(!is.na(Secretome), !is.na(Occ_bin)),
                      aes(x = Occ_bin, y = RelSed.mean, group = interaction(Occ_bin))) +
  geom_hline(yintercept=0,linetype="dotted")+
  ggrastr::rasterize(dpi = 600,
                     geom_point(shape=16, alpha=0.2, size = 0.5, color="grey50", position=position_jitter(width = 0.2))) +
  geom_violin(alpha = 0.5, linewidth = 0, width = 0.6) +
  geom_violin(fill=NA,alpha=1,draw_quantiles = c(0.5),size=0.25, width = 0.6) +
  labs(x = "Ribosome occupancy quintile", y = "Relative\nsedimentation at 30°C\n(rSed, SDs)") +
  facet_grid(.~Secretome) +
  ggsignif::geom_signif()
p_secretome_Occ_facet
```
...
## hac1 rSed vs Occ
```{r}
df <- df_Zsup_Poly_minfilt %>% filter(ORF=="YFL031W") %>%
  mutate(lab = case_when(Stress_label%in%c("30C","mock")~"unstressed",
                         TRUE~Stress_label)) %>%
  filter(Stress_group!="HS.20minPoly")
cols <- append(stress.cols,c("unstressed"="black"))
p_hac1 <- ggplot(df,
       aes(x=Rel_Occ,y=RelSed.mean,color=lab))+
  geom_point(size=2)+
  scale_color_manual(values=cols,breaks=c("unstressed","42C","46C","Azide 0.8%","Ethanol 7.5%"),
                     labels=c("unstressed","42\u00B0C","46\u00B0C","0.8% Azide","7.5% EtOH"),name="")+
  labs(x="Rib. Occ.",y="rSed (SDs)")+
  geom_text(inherit.aes=F,x=0.2,y=0.5,label="HAC1",size=10*0.35)+
  coord_cartesian(x=c(0,1),y=c(-0.75,3))+
  scale_x_continuous(breaks=c(0,0.5,1),labels=signif(c(0,0.5,1),digits=1))
p_hac1  
ggsave("../figures/Supplemental/hac1_RelSed_Occ.pdf",egg::set_panel_size(p=p_hac1+labs(x="",y=""),width=unit(35,"mm"),height=unit(35,"mm")),
       width=75,height=75,units="mm",bg="transparent")
```


# occupancy of condensing transcripts
```{r}
df <- df_Zsup_Poly_minfilt %>%
  filter(Stress_group == "Heat Shock") %>%
  group_by(ORF) %>%
  filter(length(esc.mean[Temperature == "42C"]) == 1) %>%
  mutate(esc.42C = esc.mean[Temperature == "42C"]) %>%
  mutate(esc.42C = esc.mean[Temperature == "42C"]) %>%
  ungroup %>%
  mutate(esc.bin = cut_number(esc.42C, n = 5, labels = paste0("q",c(1:5))))
df_sum <- df %>%
  group_by(Temperature,esc.bin) %>%
  summarise(Occ.mean = exp(mean(log(Occ.odds.mean),na.rm=T)))

p_42C_Occ_pSup <- ggplot(df %>% filter(Temperature == "42C"),
       aes(x=pSup.mean, y = Occ.odds.mean)) +
  ggrastr::rasterize(dpi = 600, geom_point(alpha = 0.2, shape = 16, size = 0.5)) +
  scale_y_log10nice() +
  coord_cartesian(ylim = c(0.25,50)) +
  labs(x = "Proportion in\Supernatant", y = "Ribosome Occupancy\n(odds)") +
  geom_smooth(method = "lm", se= F)
p_42C_Occ_pSup
ggsave("../figures/Supplemental/42C_Occ_pSup.pdf",
       egg::set_panel_size(p=p_42C_Occ_pSup,width=unit(35,"mm"),height=unit(35,"mm")))
```
