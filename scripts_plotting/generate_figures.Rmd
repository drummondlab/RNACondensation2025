...
kho = read_tsv("../src/khong-parker-2017-yeast-sg-transcriptome-tableS2.txt", comment='#')

dat_azide = df_Zsup_mean |> filter(Treatment_group=='azide3') |> mutate(length=LengthTxEst)
pskho = dat_azide |> filter(Treatment=='Azide_0.8%_pH6.8') |> select(ORF, length, pSup.mean, sed.mean, esc.mean) |> left_join(kho |> select(-gene, -length), by='ORF')
pskho_mock = dat_azide |> filter(Treatment=='mock') |> select(ORF, length, pSup.mean, sed.mean, esc.mean) |> left_join(kho |> select(-gene, -length), by='ORF') 
...
df = df_Zsup_mean |> filter(Treatment_group %in% c('EW_TSPP','uprInduction'), !(Treatment %in%c('42CR','mock'))) |> select(ORF, gene, Treatment, Temperature, Total.TPM.mean)
...
dfw = df |> pivot_wider(id_cols=c(ORF,gene), values_from=Total.TPM.mean, names_from=Treatment,names_prefix = 'tpm.') |> mutate(`42C`=tpm.42C/tpm.30C, `46C`=tpm.46C/tpm.30C, DTT=tpm.DTT_10mM/tpm.mock)

dat = dfw |> pivot_longer(cols=c('42C','46C','DTT'), values_to="fc", names_to='Treatment') # |> select(-tpm.30C, -tpm.42C, -tpm.46C, -tpm.mock, -tpm.DTT_10mM)

dg_fc_heat = ggplot(dat, aes(x=Treatment, y=fc)) + 
  ...
  geom_point(data=dat |> filter(gene %in% target_genes), size=2, colour=blackcol, alpha=1, shape=16) +
  geom_label_repel(data=dat |> filter(gene %in% target_genes), aes(label=gene), nudge_x = 0.3, min.segment.length = 0.1, size=3, alpha=0.8, label.size=0) +
  scale_fill_manual(values=stress.cols) +
  xlab("Treatment") +
  no.legend
dg_fc_heat
...
ggsave(file.path(github_dir, "figures/FigureS2/FigS2_tiics_vs_pbs_vs_sgs.pdf"),
...
ggplot(df_sum,
       aes(x=esc.bin, y = Occ.mean, color = Temperature)) +
  geom_point()
